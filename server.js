    ...
    socket.on('admin-command', (data) => {
        ...
        switch (data.action) {
            ...
            case 'update':
                ...
                break;
            case 'update-players':
                if (data.players && (data.team === 'L' || data.team === 'R')) {
                    for (let i = 1; i <= 5; i++) {
                        const key = `p${i}${data.team}`;
                        if (typeof data.players[key] === 'string') {
                            overlayState[key] = data.players[key];
                        }
                    }
                }
                break;
            ...
            case 'score-rem':
                ...
                break;
            case 'timeout-start':
                if ((data.value === 'L' || data.value === 'R') && overlayState.timeout[data.value + '_remain'] > 0) {
...
server.on('error', (error) => {
    if (error.code === 'EADDRINUSE') {
        console.error(`Port ${PORT} is already in use. Stop the existing server or run with another port, for example: PORT=7788 node server.js`);
        process.exitCode = 1;
        return;
    }
    throw error;
