...
function setMapScore(team, format, score) {
    ...
    const points = [0, 1, 2].map((point) =>
        document.getElementById("mp" + team + point)
    );

    points.forEach((point) => point.removeAttribute("fill"));

    if (format === "bo5") {
        points.slice(0, normalizedScore).forEach((point) =>
            point.setAttribute("fill", "")
        );
    } else {
        points.slice(1, normalizedScore + 1).forEach((point) =>
            point.setAttribute("fill", "")
        );
    }
}
